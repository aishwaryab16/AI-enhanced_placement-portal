...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    if ($_POST['action'] === 'update_additional') {
        ...
        $stmt = $mysqli->prepare('UPDATE users SET resume_link = ?, additional_links = ? WHERE id = ?');
        $stmt->bind_param('ssi', $resume_link, $additional_links, $student_id);
        if ($stmt->execute()) {
        ...
        if (!empty($username)) {
            ...
            // Clear previous achievements for user
            $del = $mysqli->prepare('DELETE FROM student_achievements WHERE username = ?');
            if ($del) { $del->bind_param('s', $username); $del->execute(); $del->close(); }

            // Insert new ones
            if (!empty($items)) {
                $ins = $mysqli->prepare('INSERT INTO student_achievements (username, achievement_text) VALUES (?, ?)');
                if ($ins) {
                    foreach ($items as $txt) { $ins->bind_param('ss', $username, $txt); $ins->execute(); }
                    $ins->close();
                }
            }
...
$user_select_cols = 'id, username, role, full_name, email, branch, semester, cgpa, backlogs, profile_photo, linkedin, github, portfolio, bio, phone, permanent_address, temporary_address, father_phone, aadhar_number, gender, category, year_of_passing, resume_link, additional_links, is_placed, college';
$has_photo_url = false;
...
if (!empty($student['username'])) {
    ...
    $stmt = $mysqli->prepare('SELECT achievement_text FROM student_achievements WHERE username = ? ORDER BY created_at DESC');
    if ($stmt) {
        $stmt->bind_param('s', $student['username']);
        $stmt->execute();
        $res = $stmt->get_result();
    ...
    <div class="main-wrapper">
        ...
        <div class="content-grid">
                ...
                <div class="card" style="border-radius: 10px; box-shadow: 0 2px 15px rgba(0,0,0,0.05); margin-bottom: 0;">
                    ...
                    <div class="info-card">
                        ...
                        <div class="info-section">
                            ...
                            <div class="info-item">
                                <div class="info-label"><i class="fas fa-map-marker-alt"></i> Current</div>
                                <div class="info-value"><?php echo !empty($student['temporary_address']) ? htmlspecialchars($student['temporary_address']) : '—'; ?></div>
                            </div>
                            <div class="info-item">
                                <div class="info-label"><i class="fas fa-map-pin"></i> Permanent</div>
                                <div class="info-value"><?php echo !empty($student['permanent_address']) ? htmlspecialchars($student['permanent_address']) : '—'; ?></div>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <form id="additionalForm" method="POST">
                        ...
                        <div class="form-grid">
                            <div class="form-group full-width">
                                ...
                                <input type="url" name="resume_link" value="<?php echo htmlspecialchars($student['resume_link'] ?? ''); ?>" disabled placeholder="https://example.com/resume.pdf">
                            </div>
                            <div class="form-group full-width">
                                <label>Additional Links</label>
                                <input type="text" name="additional_links" value="<?php echo htmlspecialchars($student['additional_links'] ?? ''); ?>" disabled placeholder="https://github.com/username, https://linkedin.com/in/username">
                            </div>
                        </div>
                ...
                <div class="card">
                    ...
                    <form id="achievementsForm" method="POST">
                        ...
                        <div style="display: flex; flex-direction: column; gap: 12px;">
                            <?php $max = max(3, count($achievements)); for ($i=0; $i<$max; $i++): $val = $achievements[$i] ?? ''; ?>
                                <div style="display: flex; align-items: center; gap: 12px; padding: 12px; background: #f9fafb; border-radius: 10px;">
                                    ...
                                    <input type="text" name="achievement_<?php echo $i; ?>" value="<?php echo htmlspecialchars($val); ?>" disabled style="flex:1; border:none; background:transparent; color:#1f2937; font-weight:600; font-size:14px;">
                                </div>
                            <?php endfor; ?>
                        </div>
                ...
                <div id="projectModal" style="display:none; position: fixed; inset: 0; background: rgba(0,0,0,0.5); z-index: 1000; align-items: center; justify-content: center;">
                    <div style="background:#fff; width: 640px; max-width: 95vw; border-radius: 12px; box-shadow: 0 10px 30px rgba(0,0,0,0.2);">
                        <div style="display:flex; align-items:center; justify-content: space-between; padding:16px 20px; border-bottom:1px solid #eee;">
                            <h3 style="margin:0; font-size:18px; color:#1f2937;">Add Project</h3>
            ...
            <div class="right-sidebar">
                ...
                <!-- Quick Actions -->
                <div class="card">
                    <h3 style="margin-bottom: 15px; font-size: 16px; color: #1f2937;">Quick Actions</h3>
                    <div style="display: flex; flex-direction: column; gap: 10px;">
                        <a href="resume_builder.php" class="btn btn-primary" style="text-decoration: none; justify-content: center; color: #5b1f1f !important;">
                            <i class="fas fa-file-alt" style="color: #5b1f1f !important; margin-right: 8px;"></i> Build Resume
                        </a>
                        <a href="career_advisor.php" class="btn btn-primary" style="text-decoration: none; justify-content: center; color: #5b1f1f !important;">
                            <i class="fas fa-robot" style="color: #5b1f1f !important; margin-right: 8px;"></i> Career Advisor
                        </a>
                        <a href="ai_interview.php" class="btn btn-primary" style="text-decoration: none; justify-content: center; color: #5b1f1f !important;">
                            <i class="fas fa-microphone" style="color: #5b1f1f !important; margin-right: 8px;"></i> AI Interview
                        </a>
                        <a href="calendar.php" class="btn btn-secondary" style="text-decoration: none; justify-content: center; color: #5b1f1f !important;">
                            <i class="fas fa-calendar" style="color: #5b1f1f !important; margin-right: 8px;"></i> My Calendar
                        </a>
                        <a href="resources.php" class="btn btn-secondary" style="text-decoration: none; justify-content: center; color: #5b1f1f !important;">
                            <i class="fas fa-briefcase" style="color: #5b1f1f !important; margin-right: 8px;"></i> Company Resources
                        </a>
                    </div>
                </div>
            </div>

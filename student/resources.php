...
#company-loading-overlay.active {
    display: flex;
}
...
.company-loader-dots span:nth-child(2) {
    animation-delay: 0.15s;
}

.company-loader-dots span:nth-child(3) {
...
<div id="company-loading-overlay">
    <div class="company-loader-card">
        <div class="company-loader-status" id="companyLoaderStatus">Opening company insight…</div>
        <div class="company-loader-dots">
            <span></span>
            <span></span>
            <span></span>
        </div>
        <div class="company-loader-subtext" id="companyLoaderSubtext">Securing data channel</div>
    </div>
</div>
...
async function searchCompanies() {
    ...
    if (cachedEntry && (now - cachedEntry.timestamp) < SEARCH_CACHE_TTL_MS) {
        displayCompanies(cachedEntry.data, cachedEntry.message, true);
    ...
    try {
        ...
        const message = data && data.message ? data.message : (companies.length ? `Found ${companies.length} companies` : 'No companies found matching your search');

        displayCompanies(companies, message, false);
...
function displayCompanies(companies, message, fromCache = false) {
    ...
    if (fromCache) {
        infoText.textContent += ' · showing cached results';
    }
...
function navigateToCompany(url) {
    const overlay = document.getElementById('company-loading-overlay');
    const statusEl = document.getElementById('companyLoaderStatus');
    const subtextEl = document.getElementById('companyLoaderSubtext');

    if (overlay && statusEl && subtextEl) {
        statusEl.textContent = 'Fetching company insights...';
        subtextEl.textContent = 'Connecting to placement cloud';

...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['action']) && $_POST['action'] === 'generate_resume') {
        ...
        // Save JSON resume to DB. Only structured data.
        $resume_json = json_encode($student_profile, JSON_UNESCAPED_UNICODE);
        if (!empty($resume_json) && !empty($username)) {
            $stmt = $mysqli->prepare(
                "INSERT INTO generated_resumes (username, resume_json) VALUES (?, ?)\n"
              . "ON DUPLICATE KEY UPDATE resume_json = VALUES(resume_json), created_at = CURRENT_TIMESTAMP"
            );
            if ($stmt) {
                $stmt->bind_param('ss', $username, $resume_json);
                $stmt->execute();
                $stmt->close();
            }
        }
        ...
        if (!$generated_resume) {
            ...
            // Save or update generated resume in database
            if (!empty($generated_resume) && !empty($username)) {
                $stmt = $mysqli->prepare(
                    "INSERT INTO generated_resumes (username, generated_resume) VALUES (?, ?)\n"
                  . "ON DUPLICATE KEY UPDATE generated_resume = VALUES(generated_resume), created_at = CURRENT_TIMESTAMP"
                );
                if ($stmt) {
                    $stmt->bind_param('ss', $username, $generated_resume);
                    $stmt->execute();
                    $stmt->close();
                }
            }

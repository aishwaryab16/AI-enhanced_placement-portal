...
<body>
        ...
        <div class="career-grid">
            ...
            <div class="card">
                ...
                <div class="advice-tabs">
                    <button class="tab active" onclick="switchTab('general')">General Advice</button>
                    <button class="tab" onclick="switchTab('technical')">Technical Skills</button>
                    <button class="tab" onclick="switchTab('soft')">Soft Skills</button>
                    <button class="tab" onclick="switchTab('interview')">Interview Prep</button>
                </div>
                ...
                <div class="fit-meter">
                    ...
                    <div id="targetRoleEdit" style="display: none; margin-top: 15px; padding-top: 15px; border-top: 2px solid rgba(146, 64, 14, 0.2);">
                        <form method="POST" style="display: flex; gap: 10px; flex-wrap: wrap;">
                            ...
                            <button type="submit" style="background: #10b981; color: white; border: none; padding: 8px 16px; border-radius: 6px; cursor: pointer; font-weight: 600;">
    ...
    <script>
        ...
        async function loadAdviceByType(type) {
            const prompts = {
                'general': `Provide general career advice for ${TARGET_ROLE} role`,
                'technical': `What technical skills should I focus on for ${TARGET_ROLE}?`,
                'soft': `What soft skills are important for ${TARGET_ROLE}?`,
                'interview': `How should I prepare for ${TARGET_ROLE} interviews at ${TARGET_COMPANY || 'top companies'}?`
            };
            
            showToast(`Loading ${type} advice...`, 'info');
            const response = await callAI(prompts[type]);
            
            if (response.error) {
                showToast(response.error, 'error');
            } else {
                // Display the advice (you can customize this)

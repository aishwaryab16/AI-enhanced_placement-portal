...
<head>
    ...
    <style>
        ...
        .score-summary { display:none; margin-top:16px; padding:14px; border-radius:10px; background:#ecfdf5; color:#065f46; font-weight:600; border:1px solid #a7f3d0; }
        .quiz-actions { display:flex; gap:12px; align-items:center; justify-content:flex-end; flex-wrap:wrap; margin-top:12px; }
...
<body>
        ...
        <div class="card">
            ...
            <form id="quizForm" style="display:none;">
                ...
                <div class="quiz-actions">
                    <button type="button" class="btn btn-secondary" onclick="regenerateQuiz()"><i class="fas fa-sync"></i> Regenerate</button>
                    <div class="spacer"></div>
                    <button type="button" class="btn btn-primary" onclick="gradeQuiz()"><i class="fas fa-check"></i> Check Score</button>
                    <button type="button" class="btn btn-primary" id="completeRoundBtn" onclick="completeRound()" disabled>
    ...
    const round = <?php echo json_encode($round); ?>;

    const storageKey = `aptitude_questions_${interviewId}_${encodeURIComponent(company)}_${encodeURIComponent(role)}_${encodeURIComponent(round)}`;
    ...
    async function generateQuestions(forceNew = false) {
        if (!forceNew) {
            const cached = getQuestionsFromStorage();
...
Array length MUST be 30.`;
        try {
            ...
            function getRandomQuestionsFromBank(bank, count) {
                // Shuffle and sample from the static bank
                return [...bank].sort(() => Math.random() - 0.5).slice(0, count);
            }
            const fallbackQs = getRandomQuestionsFromBank(bank, 30);
            renderQuiz(fallbackQs);
            saveQuestionsToStorage(fallbackQs);
        }
        finally {
            isGenerating = false;
        }
    ...
    function renderQuiz(questions) {
        ...
        container.innerHTML = '';
        const totalQuestions = questions.slice(0,30);
        ...
        // store to window
        window.__questions = totalQuestions;
    ...
    function gradeQuiz() {
        ...
        questions.forEach((q, idx) => {
            ...
            if (sel) {
                attempted++;
                if (sel.value.trim() === (q.answer||'').trim()) score++;
            }
        });

        if (attempted === 0) {
            alert('Please attempt at least one question before checking the score.');
            return;
        }
        ...
        if (summary) {
            ...
            summary.innerHTML = `<i class="fas fa-trophy"></i> You answered <strong>${attempted}</strong> question(s) and scored <strong>${score}</strong>. Total questions available: ${total}.`;
        }
        window.__lastScore = { score, attempted, total };
        const completeBtn = document.getElementById('completeRoundBtn');
    ...
    function completeRound() {
        if (!window.__lastScore) {
            alert('Please check your score before submitting the result.');
            return;
        }
        const { score, attempted, total } = window.__lastScore;
        const note = `AI Aptitude Score: ${score}/${attempted} attempted (out of ${total}) for ${company} (${role})`;
    ...
    function regenerateQuiz() {
        ...
        if (completeBtn) completeBtn.disabled = true;
        window.__lastScore = null;
        generateQuestions(true); // force new question set!
    }

...
<div class="modules-container">
    ...
    <div class="learning-path">
        <?php foreach ($moduleData['modules'] as $module): ?>
            <div class="module-card" onclick="showModuleLoading('<?php echo htmlspecialchars($module['title'], ENT_QUOTES); ?>'); window.location.href='module_details.php?company_id=<?php echo $company_id; ?>&company_name=<?php echo urlencode($company['company_name']); ?>&language=<?php echo urlencode($language); ?>&module_id=<?php echo $module['id']; ?>&module_title=<?php echo urlencode($module['title']); ?><?php echo ($company_id == 0 || $is_ai_suggested) ? '&ai_suggested=1' : ''; ?>';">
...
<script>
function showModuleLoading(moduleTitle) {
    const overlay = document.getElementById('loadingOverlay');
    const subtext = document.getElementById('loadingSubtext');
    if (overlay) {
        subtext.textContent = 'Loading ' + moduleTitle + '...';
        overlay.classList.add('active');
    }
}

// Show loading on page unload (when navigating away), but skip if navigating back
window.addEventListener('beforeunload', function() {
    // Check if we're navigating back - if so, skip loading overlay
    if (sessionStorage.getItem('navigatingBack') === 'true') {
        sessionStorage.removeItem('navigatingBack');
        return; // Don't show loading overlay
    }
    const overlay = document.getElementById('loadingOverlay');
    if (overlay) {
        overlay.classList.add('active');
...
// Clear the navigatingBack flag when page loads (in case of direct navigation)

...
// AI API Configuration - Leave space for OpenAI API key
$AI_API_KEY = 'YOUR_OPENAI_API_KEY_HERE'; // Replace with your actual OpenAI API key
$AI_API_ENDPOINT = 'https://api.openai.com/v1/chat/completions';

// Function to generate quiz questions using AI
function generateQuizQuestions($company_name, $industry, $interest_name, $api_key, $api_endpoint) {
    if (empty($api_key) || $api_key === 'YOUR_OPENAI_API_KEY_HERE') {
        // Return sample questions if no API key is provided
        return [
            'error' => false,
            'quiz' => getDefault20QuestionQuiz($company_name, $industry, $interest_name)
        ];
    }

    $prompt = "You are a senior technical interviewer and software engineering expert with 15+ years of experience at top tech companies. Create an ADVANCED, COMPANY-SPECIFIC technical quiz for {$company_name} in the {$industry} industry.
...
2. Focus on REAL-WORLD scenarios and production challenges
3. Test DEEP TECHNICAL KNOWLEDGE, not basic concepts
4. Include COMPANY-SPECIFIC technical challenges
5. Cover INDUSTRY-SPECIFIC technologies and patterns
6. Test PROBLEM-SOLVING and SYSTEM DESIGN skills
7. Include PERFORMANCE, SCALABILITY, and SECURITY considerations
8. Generate EXACTLY 20 diverse, challenging questions

QUESTION DISTRIBUTION (20 questions total):
- 4 System Design & Architecture questions
- 4 Performance Optimization & Scalability questions
- 3 Security & Best Practices questions
- 3 Debugging & Troubleshooting questions
- 2 Code Review & Quality Assessment questions
- 2 Industry-Specific Technical Challenges
- 2 Advanced Implementation Scenarios

For each question, provide:
- A complex, realistic technical scenario
...
Return ONLY valid JSON in this format:
{
  ...
  \"questions\": [
    {
      \"question\": \"Complex technical scenario with specific context and requirements\",
      \"options\": [
      ...
      \"topics_covered\": [\"Specific technical topics this question tests\"],
  ...
  \"preparation_notes\": \"Specific areas to focus on for {$company_name} technical interviews\"
}
...
- Focus on what senior developers actually work on
...
- Return ONLY valid JSON with EXACTLY 20 questions";
    ...
    $data = [
        'model' => 'gpt-4',
        'messages' => [
            ['role' => 'system', 'content' => 'You are a senior technical interviewer and software engineering expert with 20+ years of experience at FAANG companies. You specialize in creating advanced, company-specific technical assessments that evaluate deep technical knowledge, system design skills, and real-world problem-solving abilities. Your questions are challenging, realistic, and focused on production-level scenarios. Return only valid JSON.'],
    ...
    if ($http_code !== 200) {
        return [
            'error' => true,
            'message' => 'AI API request failed. Using sample questions.',
            'quiz' => null
        ];
    }
    ...
    if (isset($result['choices'][0]['message']['content'])) {
        ...
        if (json_last_error() === JSON_ERROR_NONE && is_array($quiz)) {
            ...
            if (isset($quiz['questions']) && count($quiz['questions']) === 20) {
                ...
                if ($needed > 0) {
                    $additional_questions = generateAdditionalQuestions($company_name, $industry, $interest_name, $needed, $api_key, $api_endpoint);
    ...
    return [
        'error' => true,
        'message' => 'Failed to parse AI response. Using sample questions.',
        'quiz' => null
    ];
}

function generateAdditionalQuestions($company_name, $industry, $interest_name, $needed, $api_key, $api_endpoint) {
    if (empty($api_key)) {
        return null;
    }
...
- Focus on REAL-WORLD scenarios and production challenges
- Include COMPANY-SPECIFIC technical challenges
- Cover INDUSTRY-SPECIFIC technologies and patterns
...
CRITICAL: Generate EXACTLY {$needed} questions. Count them before returning.";
    ...
    $data = [
        'model' => 'gpt-4',
        'messages' => [
            ['role' => 'system', 'content' => 'You are a senior technical interviewer. Generate exactly the requested number of advanced technical questions. Return only valid JSON array.'],
            ['role' => 'user', 'content' => $prompt]
        ],
        'temperature' => 0.2,
        'max_tokens' => 2000
...
function getDefault20QuestionQuiz($company_name, $industry, $interest_name) {
    return [
        'title' => "Advanced Technical Assessment - {$company_name} {$interest_name} Role",
        'description' => "Comprehensive technical evaluation covering advanced concepts, system design, and industry-specific challenges",
        'difficulty' => 'Advanced/Expert',
...
$quizData = generateQuizQuestions($company_name, $industry, $interest_name, $AI_API_KEY, $AI_API_ENDPOINT);
...
if ($quizData['error']) {
    ...
    $quizData = [
        'error' => false,
        'quiz' => [
            'title' => "{$company_name} Knowledge Quiz",
            'questions' => [
                [
                    'question' => "What industry does {$company_name} primarily operate in?",
                    'options' => [
                        $industry,
                        'Healthcare',
                        'Finance',
                    ...
                    'correct_answer' => 0,
                    'explanation' => "{$company_name} operates in the {$industry} industry."
                ],
                [
                    'question' => "What is the most important skill for a {$interest_name} role?",
                    'options' => [
                        'Problem-solving',
                        'Communication',
                        'Technical expertise',
                    ...
                    'correct_answer' => 2,
                    'explanation' => "Technical expertise is crucial for {$interest_name} roles."
                ],
                [
                    'question' => "Which of the following is typically required for {$interest_name} positions?",
                    'options' => [
                        'Bachelor\'s degree',
                        'Master\'s degree',
                        'High school diploma',
                    ...
                    'correct_answer' => 0,
                    'explanation' => "Most {$interest_name} positions require at least a Bachelor's degree."
                ],
                [
                    'question' => "What type of work environment does {$company_name} likely provide?",
                    'options' => [
                        'Remote work',
                        'Office-based',
                        'Hybrid',
                    ...
                    'correct_answer' => 2,
                    'explanation' => "Most modern companies offer hybrid work environments."
                ],
                [
                    'question' => "What is a key responsibility of a {$interest_name}?",
                    'options' => [
                        'Managing budgets',
                    ...
                    'correct_answer' => 1,
                    'explanation' => "{$interest_name} roles focus on developing technical solutions."
                ]
            ]
        ]
    ];
...
echo json_encode([
    'success' => true,
    'quiz' => $quizData['quiz'],
    'message' => $quizData['error'] ? 'Using sample questions. Add OpenAI API key for AI-generated questions.' : 'AI-generated quiz created successfully.'
]);
?>
